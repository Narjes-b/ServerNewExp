...
# def modify_solution(current_solution):
#     if len(current_solution) <= 1:
#         # If there's only one file or none, no modification can be made
#         return current_solution
#
#     new_solution = current_solution.copy()
#     modification_type = random.choice(['swap', 'shift', 'reverse'])
#
#     if modification_type == 'swap':
...
#
#     elif modification_type == 'shift':
#         # Implement shifting logic
#         idx = random.randint(0, len(new_solution) - 2)  # -2 ensures there's a next element to shift with
#         # Shift element at idx to idx + 1
#         new_solution[idx], new_solution[idx + 1] = new_solution[idx + 1], new_solution[idx]
#
#     elif modification_type == 'reverse':
#         # Implement subset reversing logic
#         # Select a random subset to reverse
#         start, end = sorted(random.sample(range(len(new_solution)), 2))
#         # Reverse the selected subset in place
...
# def modify_solution1(current_solution, temperature):
#     if len(current_solution) <= 1:
#         return current_solution
#
#     new_solution = current_solution.copy()
#     modification_types = ['swap', 'shift', 'reverse', 'random_insert','merge']
#
#     # Example strategy: higher temperature increases the probability of more drastic changes
#     #print(temperature)
#     if temperature > 1e-400:
#
#         #modification_types += ['merge', 'split', 'substitute', 'rotate']
#
...
#
#         elif modification_type == 'shift':
#             idx = random.randint(0, len(new_solution) - 2)
#             new_solution[idx], new_solution[idx + 1] = new_solution[idx + 1], new_solution[idx]
#
#         elif modification_type == 'reverse':
#             start, end = sorted(random.sample(range(len(new_solution)), 2))
#             new_solution[start:end + 1] = reversed(new_solution[start:end + 1])
#
#         elif modification_type == 'random_insert':
#             # This is an example of a more drastic modification that might be more common at higher temperatures
#             element = new_solution.pop(random.randint(0, len(new_solution) - 1))  # Remove a random element
#             new_solution.insert(random.randint(0, len(new_solution)), element)  # Insert it at a new position
#         elif modification_type == 'merge':
#                 if len(new_solution) < 2:
#                     return new_solution
#
#                 # Randomly select an element to shift
#                 idx = random.randint(0, len(new_solution) - 1)
#
#                 # Determine new position (ensure it's different from the current one)
#                 new_idx = (idx + 1) % len(new_solution)  # Example: move to the next position, wrapping around
#
#                 # Remove the element and insert it at the new position
#                 element = new_solution.pop(idx)
#                 new_solution.insert(new_idx, element)
#
#
#     return new_solution
...
# def modify_solution11(current_solution, step_size):
#     print(current_solution)
#
#     # Assuming current_solution is a list of numerical values
#     new_solution = [
#         min(max(1, x + random.randint(-step_size, step_size)), num_files - 1)
#         for x in current_solution
#     ]
#     return new_solution
#

# def modify_solution(current_solution, step_size):
#     new_solution = current_solution[:]  # Create a copy to avoid modifying the original
#
#     # Randomly select one index to modify
#     index_to_modify = random.randint(0, len(new_solution) - 1)
#
#     # Modify the selected index within the bounds
#     new_solution[index_to_modify] = min(max(1, new_solution[index_to_modify] + random.randint(-step_size, step_size)),
...
if __name__ == "__main__":
    projects = ['ansible-collection-hardening']
    Algo = 'SA_FINAL'
    initial_temp = 10000
    cooling_rate = 0.99
    ...
    for project in projects:
        ...
        # query_IaC_file_path = '/Users/narjes/Documents/ETS/Gits/'+project+'/roles/collect_logs/tasks/collect.yml'
        ...
        # os.mkdir(Project_path + project + '/' + Algo + '/' + _31_runs)
                ...
                if count_20 > 0 and count_90 > 0:
                    ...
                    for iter in range(31):
                        ...
                        for top in tops:
                            ...
                            # print(_31_Nb_co_modified_files)
                            # print(_31_commits_hash)

                            # _31_precision = [item for sublist in _31_precision for item in sublist]
                            # #print(_31_precision)
                            # _31_recall = [item for sublist in _31_recall for item in sublist]
                            # _31_Fscore = [item for sublist in _31_Fscore for item in sublist]
                            # _31_MRR = [item for sublist in _31_MRR for item in sublist]
                            # _31_SuccessScore = [item for sublist in _31_SuccessScore for item in sublist]
                            # _31_best_solution = [item for sublist in _31_best_solution for item in sublist]
                            # _31_Co_modified_files = [item for sublist in _31_Co_modified_files for item in sublist]
                            # _31_Nb_co_modified_files = [item for sublist in _31_Nb_co_modified_files for item in
                            #                             sublist]
                            # _31_Recommended_files = [item for sublist in _31_Recommended_files for item in sublist]
                            # _31_IaC_file = [item for sublist in _31_IaC_file for item in sublist]
                            # _31_Nb_recommended_files = [item for sublist in _31_Nb_recommended_files for item in
                            #                             sublist]
                            # _31_commits_hash = [item for sublist in _31_commits_hash for item in sublist]
        ...
        #         median = statistics.median(sorted_column)
        #         medians.append(median)
        #
        #         # Find index of element with median value
        #         median_index = medians.index(median)
        #         medians_index.append(median_index)
        #
        #         # Find index of element with highest value
        #         max_value_index = max(range(len(medians)), key=medians.__getitem__)
        #
        #     return medians_index, medians
        #
        #
        # def compute_averages(lst):
        #     # Transpose the list of lists
        #     transposed = list(zip(*lst))
        #
        ...
        #
        #
        # medians_indexes, _31_precision_median = compute_medians(_31_precision)
        # medians_indexes1, _31_recall_median = compute_medians(_31_recall)
        # medians_indexes2, _31_MRR_median = compute_medians(_31_MRR)
        # medians_indexes3, _31_Fscore_median = compute_medians(_31_Fscore)
        # medians_indexes4, _31_SuccessScore_median = compute_medians(_31_SuccessScore)
        #
        # _31_precision_avg = compute_averages(_31_precision)
        # _31_recall_avg = compute_averages(_31_recall)
        # _31_MRR_avg = compute_averages(_31_MRR)
        # _31_Fscore_avg = compute_averages(_31_Fscore)
        # _31_SuccessScore_avg = compute_averages(_31_SuccessScore)
        #
        # for i in medians_indexes:
        #     _31_median_solution.append(_31_best_solution[i])
        #     _31_median_recommended_files.append(_31_Recommended_files[i])
        #
        # # print(_31_Nb_recommended_files)
        #
        # _31_Nb_recommended_files_final = _31_Nb_recommended_files[0]
        # _31_Co_modified_files_final = _31_Co_modified_files[0]
        # _31_commits_hash_final = _31_commits_hash[0]
        # _31_IaC_file_final = _31_IaC_file[0]
        #
        # # print(len(_31_Nb_recommended_files_final))
        # # print(len(_31_median_solution))
        # # print(len(_31_median_recommended_files))
        # # print(len(_31_Co_modified_files_final))
        # # print(len(_31_precision_median))
        # # print(len(_31_recall_median))
        # # print(len(_31_precision_avg))
        # # print(len(_31_MRR_avg))
        #
        # filenameFileF = f'{filenameX}_MEDIAN.csv'
        # data = {'IaC_file': _31_IaC_file_final,
        #         'Nb_recommended_files': _31_Nb_recommended_files_final,
        #         'Best_Individual': _31_median_solution,
        #         'Recommended_files': _31_median_recommended_files,
        #         'Co_modified_files': _31_Co_modified_files_final,
        #         'Commits_hash': _31_commits_hash_final,
        #         'PRECISION_File_median': _31_precision_median,
        #         'RECALL_File_median': _31_recall_median,
        #         'FSCORE_FILE_median': _31_Fscore_median, 'SUCESS_RATE_median': _31_SuccessScore_median,
        #         'MRR_File_median': _31_MRR_median, 'PRECISION_File_avg': _31_precision_avg,
        #         'RECALL_File_avg': _31_recall_avg,
        #         'FSCORE_FILE_avg': _31_Fscore_avg, 'SUCESS_RATE_avg': _31_SuccessScore_avg,
        #         'MRR_File_avg': _31_MRR_avg}
        # df = pd.DataFrame(data)
        #
        # df.to_csv(
        #     Project_path + project + '/' + Algo + '/' + str(
        #         topk) + '/' + Results_Median + '/' + filenameFileF,
        #     index=False)
        #
        #
        # ### TOP SOLUTION
        ...
        #
        #
        # highest_indices, _31_precision_high = compute_High(_31_precision)
        # highest_indices1, _31_recall_high = compute_High(_31_recall)
        # highest_indices2, _31_MRR_high = compute_High(_31_MRR)
        # highest_indices3, _31_Fscore_high = compute_High(_31_Fscore)
        # highest_indices4, _31_SuccessScore_high = compute_High(_31_SuccessScore)
        #
        # # print(highest_indices)
        #
        # for i in highest_indices:
        #     _31_highest_solution.append(_31_best_solution[i])
        #     _31_highest_recommended_files.append(_31_Recommended_files[i])
        #
        # # print(_31_Nb_recommended_files)
        #
        # _31_Nb_recommended_files_final = _31_Nb_recommended_files[0]
        ...
        # _31_commits_hash_final = _31_commits_hash[0]
        #
        # filenameFileF = f'{filenameX}_TOP.csv'
        # data = {'IaC_file': _31_IaC_file_final,
        #         'Nb_recommended_files': _31_Nb_recommended_files_final,
        #         'Best_Individual': _31_highest_solution,
        #         'Recommended_files': _31_highest_recommended_files,
        #         'Co_modified_files': _31_Co_modified_files_final,
        #         'Commits_hash': _31_commits_hash_final,
        #         'PRECISION_File_median': _31_precision_high,
        #         'RECALL_File_median': _31_recall_high,
        #         'FSCORE_FILE_median': _31_Fscore_high, 'SUCESS_RATE_median': _31_SuccessScore_high,
        #         'MRR_File_median': _31_MRR_high, 'PRECISION_File_avg': _31_precision_avg,
        #         'RECALL_File_avg': _31_recall_avg,
        #         'FSCORE_FILE_avg': _31_Fscore_avg, 'SUCESS_RATE_avg': _31_SuccessScore_avg,
        #         'MRR_File_avg': _31_MRR_avg}
        # df = pd.DataFrame(data)
        ...
        # df.to_csv(
        #     Project_path + project + '/' + Algo + '/' + str(
        #         topk) + '/' + Results_Best + '/' + filenameFileF,
        #     index=False)

        ### results at the commit level
