...
def evaluate(individual):
    ...
    ### extract avg weight of file in hist and sim matrix and path matrix
    ## path matrix
    path_values = path_sim_data[index_IaC_file - 1, 1:]
    avg_values1 = np.std(path_values)
    #avg_values1 = np.median(path_values)
    #print(avg_values1)

    ## content matrix
    content_values = content_sim_data[index_IaC_file - 1, 1:]
    avg_values2 = np.std(content_values)
    #avg_values2 = np.median(content_values)
    #print(avg_values2)

    ## avg of path and sim matrix avg

    sim_avg=(avg_values1+avg_values2)/2

    ## hist matrix
    hist_values = co_change_data[index_IaC_file - 1, 1:]
    avg_values3 = np.std(hist_values)
    #avg_values3 = np.median(hist_values)
    #print(avg_values3)

    alpha = sim_avg
    beta = avg_values3
    # Normalize alpha and beta so their sum equals 1
    total = alpha + beta
    alpha_normalized = alpha / total
    beta_normalized = beta / total

    #print("alpha",alpha_normalized)
    #print("beta",beta_normalized)
    #print("alpha+beta",alpha_normalized+beta_normalized)


    #avg_similar = (( avg_sim*np.exp(alpha_normalized)) + ( avg_similarities3*np.exp(beta_normalized)))
    #avg_similar = (avg_sim/alpha_normalized) + (avg_similarities3 /beta_normalized)
    avg_similar = avg_sim *alpha_normalized + avg_similarities3 *beta_normalized
    # if beta_normalized<alpha_normalized:
    #     avg_similar = (math.sqrt(avg_sim *alpha_normalized) + math.sqrt(avg_similarities3 *beta_normalized))/2
    # else:
    #     avg_similar = (math.sqrt(avg_sim * alpha_normalized) + math.sqrt(avg_similarities3 * beta_normalized))/2
                ...
                if count_20 > 0 and count_90 > 0:
                    ...
                    for iter in range(31):
                        ...
                        def mutate_replacement(individual, mutation_probability):
                            mutated_individual = individual[:]  # Create a copy of the individual to avoid modifying the original
                            ...
                            for i in range(len(mutated_individual)):
                                ...
                                if mutated_individual_fitness[0] < mutation_probability:  # Check if the element should be mutated
                                    new_element = random.randint(1,len(files)-1)  # Replace with a random element (adjust the range as needed)
                        ...
                        #         #print(individual.fitness.values)
                        #         if individual.fitness.values[0] < mutation_probability:  # Check if the element should be mutated
                        #             new_element = random.randint(1,len(files)-1)  # Replace with a random element (adjust the range as needed)
                        #             individual[i] = new_element
                        #
                        #     return individual,
