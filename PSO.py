...
def initialize_swarm(num_particles=30):
    swarm = [create_individual() for _ in range(num_particles)]
    # In a discrete setup, velocity could be conceptualized differently, e.g., as the tendency to change elements
    velocities = [np.zeros(len(ind)) for ind in swarm]  # This might need rethinking for discrete problems
...
def move_particle(current_position, personal_best_position, global_best_position):
    # Placeholder for a method to decide on the new position
    # Could involve blending information from personal_best_position and global_best_position
    new_position = current_position.copy()

    # Example strategy: Swap files towards personal best or global best
    if random.random() < 0.5:  # Random decision to move towards personal best or global best
        template = personal_best_position
    else:
        template = global_best_position

    # Simple example: Swap a file from current towards a file in the template
    if len(current_position) > 0 and len(template) > 0:
        swap_out_index = random.randrange(len(new_position))
        swap_in_index = random.randrange(len(template))
        new_position[swap_out_index] = template[swap_in_index]
        ...
        for iteration in range(num_iterations):
            for i in range(num_particles):
                ...
                if fitness > personal_best_scores[i]:
                    ...
                    personal_best_positions[i] = list(swarm[i])  # Copy individual

                    # Update global best
                    if fitness > global_best_score:
                        global_best_score = fitness
                        global_best_position = list(swarm[i])  # Copy individual

            # Since direct velocity update isn't applicable, consider a custom approach here for "moving" particles
            ...
            if '_(IaC)_' in file:
                ...
                if count_20 > 0 and count_90 > 0:
                    ...
                    for iter in range(31):
                        ...
                        best_ind, best_fitness = main(pop_size, num_generations,file)
                        ind=[]
